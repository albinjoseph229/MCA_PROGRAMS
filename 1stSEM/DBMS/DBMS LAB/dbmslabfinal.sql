...
select title from industryhit where genre in 'action thriller';

select title from industryhit union select title from movies;

select title from industryhit where title not in (select title from movies);

select title from movies where title not in (select title from industryhit);

select genre ,sum(gross) sum_coll from industryhit group by genre;

select genre ,count(title) title_count from industryhit group by genre;

select genre from industryhit where count(title)>3;

select genre ,count(title) title_count from industryhit group by genre having count(title) > 3;

select genre ,count(title) title_count from industryhit group by genre having count(title) > =3;

select genre ,count(title) title_count from industryhit group by genre having count(title) > 3;

select extract( year from releasedate )year,count( title) from movies group by extract(year from releasedate) having count(title) >5 ;

select certificate,count(title)totalmovies from movies group by certificate having count(title) >5;

select certificate,sum(gross)totalgross from movies group by certificate  having sum(gross) >100000000;

desc movies


SELECT 
    TO_CHAR(RELEASEDATE, 'YYYY') AS RELEASE_YEAR, 
    COUNT(*) AS MOVIE_COUNT 
FROM 
    YOUR_TABLE_NAME 
GROUP BY 
    TO_CHAR(RELEASEDATE, 'YYYY') 
ORDER BY 
    MOVIE_COUNT DESC 
...
SELECT TO_CHAR(RELEASEDATE, 'YYYY') AS Release_Year, COUNT(*) AS Movie_Count 
FROM Movies 
GROUP BY TO_CHAR(RELEASEDATE, 'YYYY') 
ORDER BY COUNT(*) DESC 
FETCH FIRST 3 ROWS ONLY;

select title, runtime from movies where metascore=(select max(metascore)from movies);

select title from movies where gross>(select avg(gross)from movies);

select title,description from movies where metascore<(select avg(metascore) from movies);

select title,imdbrating from movies where extract(year from release_date)= (select extract(year from releasedate) from movies where imdbrating=(select max(imdbrating) from movies));

select title,imdbrating from movies where extract(year from release_date)= (select extract(year from releasedate) from movies where imdbrating=(select max(imdbrating) from movies));

select title,imdbrating from movies where extract(year from release_date)= (select extract(year from release_date) from movies where imdbrating=(select max(imdbrating) from movies));

select title,imdbrating from movies where extract(year from releasedate)= (select extract(year from releasedate) from movies where imdbrating=(select max(imdbrating) from movies));
...
select title,imdb_rating from movies where extract(year from releasedate)= (select extract(year from releasedate) from movies where imdb_rating=(select max(imdb_rating) from movies));

WITH AvgRatings AS ( 
    SELECT TO_CHAR(RELEASEDATE, 'YYYY') AS Release_Year, AVG(IMDB_RATING) AS Avg_IMDB_Rating 
    FROM Movies 
    GROUP BY TO_CHAR(RELEASEDATE, 'YYYY') 
...
FROM Movies 
WHERE TO_CHAR(RELEASEDATE, 'YYYY') = (SELECT Release_Year FROM AvgRatings) 
...
select title ,gross from ( select title,gross,rank() over (order by gross desc)as ranking from movies ) where ranking =3;

select title,imdbrating from movies where metascore > 2 * imdbrating;

select title,imdb_rating from movies where metascore > 2 * imdbrating;

select title,imdb_rating from movies where metascore > 2 * imdb_rating;
...
from movies 
where to_char(releasedate, 'yyyy') = '2022';

select releaseyear,sum(votes) as total_votes 
from movies 
where to_char(releasedate, 'yyyy') = '2022';

select releasedate,sum(votes) as total_votes 
from movies 
where to_char(releasedate, 'yyyy') = '2022';
